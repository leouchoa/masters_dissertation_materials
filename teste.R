...
nus <- c(0.5, 0.5)
mu <- c(1.119256, 2.170499)
theta0 <- c(1.0, 1.0, 2.0, 0.5)
obs_vec <- log_cd
M <- optim(theta0,
           fn = BivMaternEstim:::LLike_biwm,
           gr = BivMaternEstim:::LLike_biwm_reduced_grad,
           method = "L-BFGS-B",
           ...
           obs_vec = obs_vec)
# Olhar mvtnorm
BivMaternEstim:::LLike_biwm(c(0.332101, 0.497864, 4.91074, 0.503153),
                            nus = nus,  mu = mu, # We always use Z = vec(\mathbf{Y} - boldsymbol\mu), maybe center it outside function?
                            coords_matrix = coords,
                            obs_vec = obs_vec)