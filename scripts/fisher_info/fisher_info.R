...
M_1 <- matern_cov_wrapper(d,a = as[1],nu = nus[1])
M_2 <- matern_cov_wrapper(d,a = as[2],nu = nus[2])
M_3 <- matern_cov_wrapper(d,a = as[3],nu = nus[3])
...
B_11_sigma1 = C_11_st %*% M_1 + C_12_st * rho * sqrt( sigmas[2]/(2*sigmas[1]) ) %*% M_3
B_21_sigma1 = t(C_12_st) %*% M_1 + C_22_st * rho * sqrt( sigmas[2]/(2*sigmas[1]) ) %*% M_3
...
B_12_sigma2 = C_11_st * rho * sqrt( sigmas[1]/(2*sigmas[2]) ) %*% M_3 + C_12_st %*% M_2
B_22_sigma2 = t(C_12_st) * rho * sqrt( sigmas[1]/(2*sigmas[2]) ) %*% M_3 + C_22_st %*% M_2
...
B_22_rho = sqrt( sigmas[1]* sigmas[2] ) * C_21_st %*% M_3
