...
generic_test_regular <- fit_biwm(log_cd_regular, coords_regular, initial_guess, nus_vec)
...
exp_res_regular <- 
  as.data.frame(
    rbind(
      ...
      initial_guess = round(initial_guess,5),
...
generic_test_random <- fit_biwm(log_cd_random, coords_random, initial_guess, nus_vec)
...
# ----- 04 Estimate Distribution  - Generic Guess -----


res_generic <- replicate(50,{
...
hist(res_generic[3,],xlab = "");abline(v = true_param[3], col = "red",lwd = 5)
hist(res_generic[4,],xlab = "");abline(v = true_param[4], col = "red",lwd = 5)


# ----- 04 Estimate Distribution  - Informed Guess -----


res_informed <- replicate(50,{
